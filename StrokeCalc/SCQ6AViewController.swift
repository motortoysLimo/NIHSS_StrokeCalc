...
class SCQ6AViewController: SCBaseViewController, UITableViewDataSource, UITableViewDelegate {
...
//        self.timerButton.setShadowLayer()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            if indexPath.row == 0 {
                ...
                detailCell.titleLabel.attributedText = NSMutableAttributedString().boldHeader(text: "6A. Motor - Left Leg")
                detailCell.questionInfolabel.attributedText = NSMutableAttributedString().normal(text: "Tap the LIGHTBULB to determine the patient's starting position (sitting or lying down). Test the non-parazyled leg first.\n\nWhen ready, start the ").orangeBold(text: "5-second timer").normal(text: ".")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.row != 0 || indexPath.section == 5 {
            ...
            if indexPath.section == 5 { // UN - Untestable
                ...
                testScoreService.setScoresForQuestions(dictionary: [SCQType.Q6A.rawValue: score])
                performSegue(withIdentifier: "toq6b", sender: self)
    ...
    @IBAction func mimicTapped(_ sender: Any) {
    ...
    func performNext() {
        testScoreService.setScoresForQuestions(dictionary: [SCQType.Q6A.rawValue: "UN"])
        performSegue(withIdentifier: "toq6b", sender: self)
